function [ZoneInfo,CtrlSig,MPC_DebugData]=callSim(HardwareTime,timestep,Meas,Meas_MPC)
%% Notes
%% Inputs
% timestep
...
% Meas_MPC:
%     Tg1 = Meas_MPC(1) :       Downward Grout Section Temperature [°C]
%     Tg2 = Meas_MPC(2) :       Upward Grout Section Temperature [°C]
%     T1 = Meas_MPC(3) :        Downward Water Temperature [°C]
%     T2 = Meas_MPC(4) :        Upward Water Temperature [°C]
%     acc_error = Meas_MPC(5) : Accumulated error, which is "Accum-Error-Cool" in HardwareData
% recv 
%   = 0; normal mode
...
persistent stepsize 
persistent conn CollName DBName MeasLabel ZoneInfoLabel CtrlSigLabel ZIFields CSFields Meas_MPCLabel
persistent GEB_case Control_method TES Location STD Dense_Occupancy EGRSave_Occupant Season_type SimulinkName 
persistent CollName_Location CollName_STD CollName_DenOcc
%% Initialization
if timestep==0
...
%         '_',char(datestr(now,'HHMMSS'))];
    ...
    Meas_MPCLabel={'Tg1','Tg2','T1','T2','acc_error'};
...
if (recv<0.5 || timestep>ts_recv)
    ...
    SimMPCDataDoc.DocType='SimData_MPC';
    SimMPCDataDoc.Timestep=timestep;
    SimMPCDataDoc.Time=startTime+timestep*60;
    insert(conn,CollName,SimMPCDataDoc);
    % push Meas to DB
    ...
    [CtrlSig,MPC_DebugData]= Control_Model(startTime,timestep,Season_type,GEB_case,Control_method,TES,Meas,STD,Dense_Occupancy,conn,CollName,Meas_MPC);
else
...
if timestep==0
   ...
   if Location==4  % Tucson
        set_param(['OneZone_STD',char(CollName_STD(STD)),'_',...
            char(CollName_Location(Location)),'2019Year_',char(CollName_DenOcc(Dense_Occupancy+1))],'StartTime',string(startTime_forEnergyPlus),'StopTime',string(stopTime));
        % start Simulink
        set_param(['OneZone_STD',char(CollName_STD(STD)),'_',...
            char(CollName_Location(Location)),'2019Year_',char(CollName_DenOcc(Dense_Occupancy+1))],'SimulationCommand','start');
        for i_firstday=0:(1440-1)
            set_param(['OneZone_STD',char(CollName_STD(STD)),'_',...
                char(CollName_Location(Location)),'2019Year_',char(CollName_DenOcc(Dense_Occupancy+1))],'SimulationCommand','pause');
            set_param(['OneZone_STD',char(CollName_STD(STD)),'_',...
                char(CollName_Location(Location)),'2019Year_',char(CollName_DenOcc(Dense_Occupancy+1))],'SimulationCommand','continue');
        end
    else
