...
block.InputPort(1).Dimensions        = 17;
...
persistent conn CollName ZoneInfoLabel ZoneInfoMPCLabel
...
if isempty(CollName)
    ...
    ZoneInfoMPCLabel={'Twall_f','Twall_c','Twall_e','Twall_s','Twall_w','Twall_n'};
...
%% Obtain zone info
ZoneInfo=block.InputPort(1).Data(1:end-7);
...
ZoneInfo_MPC=block.InputPort(1).Data(11:16);
...
if clock>= (DayofYear-1)*86400   % enstore ZoneInfo data only in the real running day
    if (recv<0.5 || clock>time_recv)
        ...
        ZIMPCquery=mongo2mongofiled_upset(ZoneInfoMPCLabel,ZoneInfo_MPC);
        update(conn,CollName,['{"Time":',num2str(clock),...
            ',"DocType":"SimData_MPC"}'],ZIMPCquery);
    end
end
%end Outputs
