...
for i = 1:length(mat_files)
    ...
    raw_data.Measurements = reformat(raw_data.Measurements);
    raw_data.SimData = reformat(raw_data.SimData);
    temp = reformat(raw_data.OccupantMatrix');
    ...
    % Remove first row
    Measurements = raw_data.Measurements(2:1441,:);
...
%---System data     
    ...
    for j = 1:row_last_system_data
        ...
        % Extract data
        switch raw_data_struct{1}
            case 'HardwareData'
                if ~ismember(raw_label{1}, hdata_sim.Properties.VariableNames)
                    continue;
                end
                temp = hdata_sim.(raw_label{1});
            case 'SimData'
                temp = [SimData.(raw_label{1})]';
            case 'Measurements'
                temp = [Measurements.(raw_label{1})]';
        end

        % Conversion
        if strcmp(post_unit{1}, 'DEG_F')
            post_data.(post_label{1}) = temp .* 1.8 + 32;
        else
    ...
    % Create a new column 'ground_temp_sim' in post_data table
    post_data.ground_temp_sim = zeros(height(post_data), 1);
    switch Location
        case 'Atlanta'
            post_data.ground_temp_sim(:) = 17.8;
        case 'Buffalo'
            post_data.ground_temp_sim(:) = 10;
        case 'NewYorkCity'
            post_data.ground_temp_sim(:) = 13.6;
        case 'Tucson'
            post_data.ground_temp_sim(:) = 22.7;
    end
    ...
    for j = 1:row_last_system_data
        ...
        % Skip if the post_label is not in the post_data table (2024-03-30)
        if ~ismember(post_label{1}, post_data.Properties.VariableNames)
            continue;
        end
